...
function SimpleCryptoView(props) {
    const [changePercent] = useState(() => {
        let number = ((props.data.regularMarketPrice*props.asset.amount-props.asset.totalSpent)/props.asset.totalSpent*100)
	...
	return (
        <div className="simple-crypto-view row center">
            <div className='sub-section row space-between h-center'>
                ...
                <p className="simple-crypto-view-item">{format(props.asset.amount)} {props.asset.name.toString().toUpperCase()}</p>
